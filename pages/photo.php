...
class Flogr_Photo extends Flogr_Page {
    ...
    function get_userlink($photoId = null) {
        $p = new Profiler();
        return "http://www.flickr.com/photos/{$this->get_userId($photoId)}/";
    }
    ...
    function get_permalink($photoId = null) {
        ...
        return MODULE_INDEX."&photoId={$this->get_photo_id($photoId)}";
    }
    ...
    function get_tags_list($photoId = null, $before = '<li>', $after = '</li>') {
        ...
        while ($tok !== false) {
        	...
        	$tag_list .= "{$before}<a href='".MODULE_INDEX."&type=recent&tags={$tok}'>{$tok}</a>{$after}";
            $tok = strtok(" ");
        }
